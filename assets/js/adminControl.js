jQuery(function ($) {
   ...
   $(document).ready(function () {
      ...
      const findString = 'page=elv-samba-helper-admin-page'
      if (window.location.href.includes(findString)) {

         // Fix admin menus (TODO fix this properly later?)
         (function () {

            const currentlySelectedTab = window['elvSambaHelpercurrentlySelectedTab']
            ...
            const sambaMenuWrapper = $('#toplevel_page_elv-samba-helper-admin-page')
         ...
         const createAdminWidget = function () {
            ...
            const createAdminWidget_AJAX = function (wrapper, triggerInput) {
               ...
               $.ajax({
                  ...
                  success: function (response) {
                     wrapper.append(`<tr class="elv_sambaHelper_widgetLine js-widgetLine" data-id="${response}">
              <td>
              </td>
              <td>
              ...
              <td class="elv_sambaHelper_widgetControlTd">
                 <a href="${location.href}&widgettabid=${response}">
                   <button class="button button-primary button-medium">${elvSambaHelperTranslations.shortcodeDynamicEdit}</button>
                 </a>
              </td>
              <td class="elv_sambaHelper_widgetControlTd">
                <button class="button button-secondary button-medium js-deleteWidget-trigger">${elvSambaHelperTranslations.shortcodeDynamicRemove}</button>
              </td>
            </tr>`)
                  },
                  error: function (request, status, error) {
         ...
         const XMLGeneration_finish = function () {
            ...
            const productMessage = `${elvSambaHelperTranslations.reportProducts} ${(exportReports.products) ? elvSambaHelperTranslations.feedSuccess : elvSambaHelperTranslations.feedFailure}`
            //const productCategoriesMessage = `${elvSambaHelperTranslations.reportProductCategories} ${(exportReports.productCategories) ? elvSambaHelperTranslations.feedSuccess : elvSambaHelperTranslations.feedFailure}`
            const customersMessage = `${elvSambaHelperTranslations.reportCustomers} ${(exportReports.customers) ? elvSambaHelperTranslations.feedSuccess : elvSambaHelperTranslations.feedFailure}`
            const ordersMessage = `${elvSambaHelperTranslations.reportOrders} ${(exportReports.orders) ? elvSambaHelperTranslations.feedSuccess : elvSambaHelperTranslations.feedFailure}`
