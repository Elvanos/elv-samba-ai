...
function checkExistingPassword($forceRewrite = false) {
  if (!get_option('sambaHelperPrehash')) {
    add_option('sambaHelperPrehash', bin2hex(openssl_random_pseudo_bytes(16)));
  } else if ($forceRewrite) {
    update_option('sambaHelperPrehash', bin2hex(openssl_random_pseudo_bytes(16)));
  }
}

function generateAccessFile() {
  $file = fopen(WP_CONTENT_DIR . "/sambaHelperExport/.htaccess", "w");
  ...
  fwrite($file, "AuthUserFile " . WP_CONTENT_DIR . "/sambaHelperExport/.htpasswd\n");
...
function generatePasswordFile($ajaxMode = false) {
  $pass = get_option('sambaHelperPrehash');
  $hash = generatePasswordHash($pass);

  $file = fopen(WP_CONTENT_DIR . "/sambaHelperExport/.htpasswd", "w");

  fwrite($file, "sambaHelper:" . $hash);
  fclose($file);
